            ...
            while (!finished) {
                ...
                switch (DNSLabel.labelForByte(len)) {
                    ...
                    case Extended:
                        ...
                        Log.v("-----","Extended label are not currently supported.");
                        break;
                    case Unknown:
                    default:
                        Log.v("-----","Unsupported DNS label type: '{}'"+Integer.toHexString(len & 0xC0) );
                }
            }
...
//            );
            ...
            Log.v("-----","DNSIncoming() dump " + print(true) + "\n exception ", e);
    ...
    private DNSQuestion readQuestion() {
        ...
        if (type == DNSRecordType.TYPE_IGNORE) {
            Log.v("-----","Could not find record type: {}"+this.print(true));
        }
    ...
    private DNSRecord readAnswer(InetAddress source) {
        ...
        DNSRecordType type = DNSRecordType.typeForIndex(_messageInputStream.readUnsignedShort());
        if (type == DNSRecordType.TYPE_IGNORE) {
            //Log.v("-----","Could not find record type. domain: {}\n{}", domain, this.print(true));
        }
        int recordClassIndex = _messageInputStream.readUnsignedShort();
        ...
        if ((recordClass == DNSRecordClass.CLASS_UNKNOWN) && (type != DNSRecordType.TYPE_OPT)) {
            //Log.v("-----","Could not find record class. domain: {} type: {}\n{}", domain, type, this.print(true));
        }
        boolean unique = recordClass.isUnique(recordClassIndex);
        ...
        switch (type) {
            ...
            case TYPE_AAAA: // IPv6
                ...
                if (isIPv4MappedIPv6Address(incomingBytes)) {
                    Log.v("-----","AAAA record with IPv4-mapped address for {}"+domain);
                } else {
            ...
            case TYPE_PTR:
                ...
                if (service.length() > 0) {
                   ...
                   // Log.v("-----","PTR record of class: {}, there was a problem reading the service name of the answer for domain: {}", recordClass, domain);
                }
                break;
            case TYPE_TXT:
                ...
                if (version == 0) {
                    ...
                    while (_messageInputStream.available() > 0) {
                        ...
                        if (_messageInputStream.available() >= 2) {
                            ...
                            Log.v("-----","There was a problem reading the OPT record. Ignoring.");
                            break;
                        }
                        int optionLength = 0;
                        if (_messageInputStream.available() >= 2) {
                        ...
                        switch (optionCode) {
                            case Owner:
                                ...
                                try {
                                    ...
                                    Log.v("-----","Malformed OPT answer. Option code: Owner data: {}"+this._hexString(optiondata));
                                }
...
//                                }
                            ...
                            case Unknown:
                                if (optionCodeInt >= 65001 && optionCodeInt <= 65534) {
                                    ...
                                    //Log.v("-----","There was an OPT answer. Not currently handled. Option code: {} data: {}", optionCodeInt, this._hexString(optiondata));
                                }
                                break;
                            default:
                   ...
                   // Log.v("-----","There was an OPT answer. Wrong version number: {} result code: {}", version, extendedResultCode);
                }
                break;
            default:
