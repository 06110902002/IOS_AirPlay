    ...
    boolean suppressedBy(DNSIncoming msg) {
        try {
            ...
            return false;
             ...
             Log.v("-----","suppressedBy() message " + msg + " exception ", e);
            // msg.print(true);
            return false;
        }
    }
    ...
    public static abstract class Address extends DNSRecord {
        ...
        protected Address(String name, DNSRecordType type, DNSRecordClass recordClass, boolean unique, int ttl, byte[] rawAddress) {
            ...
            try {
                ...
                Log.v("-----","Address() exception "+exception.toString());
            }
        }
        ...
        boolean sameValue(DNSRecord other) {
            try {
                ...
                Log.v("-----","Failed to compare addresses of DNSRecords", e);
                return false;
            }
        }
            ...
            if (dns.getLocalHost().conflictWithRecord(this)) {
                ...
                if (localAddress != null) {
                    ...
                    if (comparison == 0) {
                        ...
                        Log.v("-----","handleQuery() Ignoring an identical address query");
                        return false;
                    }

                    Log.v("-----","handleQuery() Conflicting query detected.");
                    // Tie breaker test
        ...
        boolean handleResponse(JmDNSImpl dns) {
            if (dns.getLocalHost().conflictWithRecord(this)) {
                Log.v("-----","handleResponse() Denial detected");
    ...
    public static class Service extends DNSRecord {
        ...
        boolean handleQuery(JmDNSImpl dns, long expirationTime) {
            ...
            if (info != null && (info.isAnnouncing() || info.isAnnounced()) && (_port != info.getPort() || !_server.equalsIgnoreCase(dns.getLocalHost().getName()))) {
                Log.v("-----","handleQuery() Conflicting probe detected from: {}"+getRecordSource());
                ...
                try {
                    if (dns.getInetAddress().equals(getRecordSource())) {
                        Log.v("-----","Got conflicting probe from ourselves\nincoming: {}\nlocal   : {}"+this.toString()+localService.toString());
                    }
                } catch (IOException e) {
                    Log.v("-----","IOException", e);
                }
                ...
                if (comparison == 0) {
                    ...
                    // own records come in on different interfaces than the ones they were sent on.
                    // see section "10. Conflict Resolution" of mdns draft spec.
                    Log.v("-----","handleQuery() Ignoring a identical service query");
                    return false;
                }
                ...
                if (info.isProbing() && comparison > 0) {
                    ...
                    Log.v("-----","handleQuery() Lost tie break: new unique name chosen:{}"+info.getName());
        ...
        boolean handleResponse(JmDNSImpl dns) {
            ...
            if (info != null && (_port != info.getPort() || !_server.equalsIgnoreCase(dns.getLocalHost().getName()))) {
                ...
                if (info.isProbing()) {
                    ...
                    dns.getServices().remove(oldName);
                    dns.getServices().put(info.getQualifiedName().toLowerCase(), info);
                    Log.v("-----","handleResponse() New unique name chose:{}"+info.getName());
