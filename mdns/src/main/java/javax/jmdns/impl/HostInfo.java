    ...
    public static HostInfo newHostInfo(InetAddress address, JmDNSImpl dns, String jmdnsName) {
        ...
        try {
            if (addr == null) {
                ...
                if (addr.isLoopbackAddress()) {
                    Log.v("-----","Could not find any address beside the loopback.");
                }
            }
            if (aName.length() == 0) {
            ...
            Log.v("-----","Could not initialize the host network interface on " + address + "because of an error: " + e.getMessage(), e);
    ...
    private HostInfo(final InetAddress address, final String name, final JmDNSImpl dns) {
        ...
        if (address != null) {
            try {
                ...
                Log.v("-----","LocalHostInfo() exception ", exception);
            }
        }
    }
