...
public class ByteWrangler {
    private static final String TAG = "ByteWrangler";
    /**
            ...
            try {
                ...
                for (final Map.Entry<String, ?> entry: props.entrySet()) {
                    ...
                    if (data.length > MAX_VALUE_LENGTH) {
                        Log.v(TAG,"Cannot have individual values larger that 255 chars. Offending value: {}"+key + (val == null ? "" : "=" + val));
                        return EMPTY_TXT;
                    }
    ...
    public static byte[] encodeText(final String text) throws IOException {
        ...
        if (data.length > MAX_VALUE_LENGTH) {
            Log.v(TAG,"Cannot have individual values larger that 255 chars. Offending value: {}"+text);
            return EMPTY_TXT;
        }
        out.write((byte) data.length);
        out.write(data, 0, data.length);
